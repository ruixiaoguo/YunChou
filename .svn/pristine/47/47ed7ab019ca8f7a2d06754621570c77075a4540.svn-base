...
public class LTPageTitlesView: UIView {
    ...
    var isClickScrollAnimation = true
    ...
    private lazy var sliderLineView: UIView = {
        ...
        let sliderLineView = UIView(frame: CGRect(x: layout.lrMargin, y: bounds.height - layout.bottomLineHeight - layout.pageBottomLineHeight - layout.toBottomHeight, width: 0, height: layout.bottomLineHeight))
...
extension LTPageTitlesView {
    ...
    private func setupButtonsLayout() {
        ...
        var Y: CGFloat = 0
        var toTopHeight: CGFloat = 0
        
        if layout.isShowBackImage {
            //有图片设置固定、亦可设定其他
            for _ in titles.enumerated(){
                glt_textWidths.append(iPhone5 ? kWidth(R: 80) : 80)
                glt_lineWidths.append(iPhone5 ? kWidth(R: 80) : 80)
            }
        ...
        let subH = bounds.height - toTopHeight
        
        for index in 0..<titles.count {
            let subW = glt_textWidths[index]
            let buttonReact = CGRect(x: upX, y: Y, width: subW, height: subH)
        ...
        sliderScrollView.bringSubviewToFront(sliderLineView)
...
extension LTPageTitlesView: LTPageViewDelegate {
...
extension LTPageTitlesView {
    ...
    private func subButton(frame: CGRect, flag: Int, title: String?, parentView: UIView) -> UIButton {
        ...
        button.tag = flag
        if layout.isShowBackImage{
            button.setBackgroundImage(UIImage.init(named: "btn1"), for: .normal)
