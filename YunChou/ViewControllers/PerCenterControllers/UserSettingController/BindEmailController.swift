...
class BindEmailController: BaseController {
    ...
    @objc func overClick(_ button:UIButton){
        ...
        //MARK:===================绑定邮箱
        YCUserCenterNetwork.request(target: .ModifyUserInfo(userId: USERID(), nickName: "", weixin: "", email: emailTextFild.text!), success: { (JSON) in
            print("=========\(JSON)")
            if(JSON["msg"]["code"].rawString()=="000000"){
                //更新本地存储
                UserInfoManager.shared.infoModel.email = self.emailTextFild.text!
