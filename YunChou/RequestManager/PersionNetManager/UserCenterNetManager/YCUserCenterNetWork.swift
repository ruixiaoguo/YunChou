...
extension userCenterApiManager:TargetType{
    ...
    var task: Task {
        ...
        case let .gaintUserInfo(userId): // 带有参数,注意前面的let
            ...
            let dic = YCParameManeger.shared.transCode(code: "USER1017", dic: requestParam!)
        ...
        case let .ModifyUserInfo(userId,nickName,userImg,weixin,email): // 带有参数,注意前面的let
            ...
            requestModel.userId = userId
            let requestParam = requestModel.toJSON()
            let dic = YCParameManeger.shared.transCode(code: "USER1006", dic: requestParam!)
            print("===\(String(describing: dic))")
            return .requestParameters(parameters: dic,
                                      encoding: URLEncoding.default)
        case let .setNewPassWord(userId,loginPwd): // 带有参数,注意前面的let
            ...
            let requestParam = requestModel.toJSON()
            let dic = YCParameManeger.shared.transCode(code: "USER1003", dic: requestParam!)
            print("===\(String(describing: dic))")
            return .requestParameters(parameters: dic,
                                      encoding: URLEncoding.default)
        case let .modifyPassWord(phone,loginPwd,vaCode): // 带有参数,注意前面的let
            ...
            let requestParam = requestModel.toJSON()
            let dic = YCParameManeger.shared.transCode(code: "USER1008", dic: requestParam!)
            print("===\(String(describing: dic))")
            return .requestParameters(parameters: dic,
                                      encoding: URLEncoding.default)
        }
